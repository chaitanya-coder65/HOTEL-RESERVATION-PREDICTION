...
class DataProcessor:
    ...
    def select_feature(self,df):
        try:
            ...
            logger.info(f"Feature selected{top_10_feature}")
            top_10_df = df[top_10_feature.tolist() + ["booking_status"]]

            logger.info("Feature selection Completed ")
            return top_10_df
        except Exception as e:
            logger.error(f"Error During feature selecting Step {e}")
            raise CustomException("Error while feature selecting",e)
    ...
    def process(self):
        try:
            ...
            raise CustomException("Error while day=ta preprocessing pipeline",e)
